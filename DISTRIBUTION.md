...
sha256 검증 → `~/Applications/NoiseCleaner`에 설치 → `~/.local/bin/noise-cleaner`
심링크 → 방어적 quarantine 제거. 이후 `noise-cleaner`로 실행.

환경변수로 소스/위치 조정: `NC_URL`, `NC_SHA256`, `NC_PREFIX`, `NC_BIN`, `NC_RELEASE`.

### 2) Homebrew cask

별도 탭(`devKangMinHyeok/homebrew-tap`)에 `packaging/homebrew/Casks/noise-cleaner.rb`
를 두면:
...
`noise-cleaner` CLI가 PATH에 심링크된다.
...
번들 빌드 → `dist/release/NoiseCleaner-macos-arm64.tar.gz` → **sha256 출력**.
...
- `noise-cleaner` CLI: 심링크가 어느 위치에서든 번들 런처를 정확히 실행,
  인자 전달, 경로 독립 (실측)
